...
ROOT_DIRS="default"      # ...and these directories       (space separated list)
...
for CHROME_SUBDIR in $CHROME_PROVIDERS; do
  find $CHROME_SUBDIR -path '*CVS*' -prune -o -type f -print | grep -v \~ | grep -v /\\. >> files
done
...
for DIR in $ROOT_DIRS; do
  mkdir $TMP_DIR/$DIR
  FILES="`find $DIR -path '*CVS*' -prune -o -type f -print | grep -v \~ | grep -v /\\.`"
  echo $FILES >> files
